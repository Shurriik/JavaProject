...
public class TableRepository 
{
    ...
    public List<Table> findByFilter(Integer minCapasity, String location, Boolean available)
    {
        List<Table> result = new ArrayList<>();
        for (Table table : tables)
        {
            ...
            if (minCapasity != null && table.getCapacity() < minCapasity)
            {
                matches = false;
            }
            if (matches && location != null && !location.isEmpty() && !table.getLocation().equalsIgnoreCase(location))
            {
                matches = false;
            }
            if (matches)
