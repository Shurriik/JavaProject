...
public class TableService 
{
    ...
    public TableUserResponse createTableForUser(TableUserRequest request)
    {
        ...
        {
            throw new RuntimeException("Стол с таким номеров" + request.getNumber() + "уже существует");
        }
    ...
    {
        Table table = tableRepository.findById(id).orElseThrow(() -> new RuntimeException("Стол с таким id" + id + "не найден"));
    ...
    {
        Table table = tableRepository.findByNumber(number).orElseThrow(() -> new RuntimeException("Стол с таким номером" + number + "не найден"));

        return tableMapper.toUserResponse(table);
    }
    
    public List<TableUserResponse> getTableByFilterForUser(Integer minCapacity, String location, boolean available)
    {
        List<Table> tables = tableRepository.findByFilter(minCapacity, location, null);
        return tableMapper.toUserResponseList(tables);
    ...
    public TableAdminResponse createTableForAdmin(TableAdminRequest request)
    {
        if (tableRepository.existsByNumber(request.getNumber()))
        {
            throw new RuntimeException("Стол с таким номером" + request.getNumber() + "уже существует");
        }
    ...
    {
        Table existingTable = tableRepository.findById(id).orElseThrow(() -> new RuntimeException("Стол с таким id" + id + "не найден"));
        if (!existingTable.getNumber().equals(request.getNumber()))
        {
            if (tableRepository.existsByNumber(request.getNumber()))
            {
                throw new RuntimeException("Стол с таким номером" + request.getNumber() + "не найден");
            }
        }
        tableMapper.updateEntityFromAdminRequest(existingTable, request);
        Table updatedTable = tableRepository.save(existingTable);
        return tableMapper.toAdminResponse(existingTable);

    }

    public void softDeletedTableForAdmin(Long id)
    {
        if(!tableRepository.existsById(id))
        {
            throw new RuntimeException("Стол с таким id" + id + "не найден");
        }
        tableRepository.restoredById(id);
    }

    public void restoredTableForAdmin(Long id)
    {
        if (!tableRepository.existsById(id))
        {
            throw new RuntimeException("Стол с таким id" + id + "не найден");
    ...
    public void hardDeletedByIdForAdmin(Long id)
    {
        if (!tableRepository.existsById(id))
        {
            throw new RuntimeException("Стол с таким id" + id + "не найден");
